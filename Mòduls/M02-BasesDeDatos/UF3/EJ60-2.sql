DROP DATABASE IF EXISTS banco;
CREATE DATABASE banco;
USE banco;
CREATE TABLE moviment
(compte VARCHAR(10) NOT NULL,
data datetime NOT NULL DEFAULT now(),
descripcio VARCHAR(30) NOT NULL,
tipus ENUM('H','D') NOT NULL,
valor DECIMAL(7,2) NOT NULL
);
CREATE TABLE seguretat
(compte VARCHAR(10) NOT NULL,
data TIMESTAMP NOT NULL,
usuari VARCHAR(20) NOT NULL,
descripcio_valor VARCHAR(30) NOT NULL
);
CREATE TABLE saldo
(compte VARCHAR(10) PRIMARY KEY,
saldo DECIMAL(7,2) NOT NULL
);
CREATE TABLE saldo_diari
(data  DATE PRIMARY KEY,
saldo DECIMAL(7,2) NOT NULL
);
DELIMITER //
CREATE TRIGGER inserte after insert ON moviment FOR EACH ROW
BEGIN 
declare descr VARCHAR(40);
declare saldo_Anterior decimal (7,2);
set saldo_Anterior = (select saldo from saldo_diari where data < DATE(new.data) order by data desc limit 1);
if ((select count(*) from saldo where compte = new.compte) = 0) then
insert into saldo value (new.compte, 0);
if(new.tipus = "H") then
insert into saldo values (new.compte, new.saldo);
else 
insert into saldo values (new.compte, -new.saldo);
end if;
else
if(new.tipus = "H") then
update saldo set saldo = saldo + new.valor where compte = new.compte;
else 
update saldo set saldo = saldo - new.valor where compte = new.compte;
end if;
end if;
if ((select count(*) from saldo_diari where data = new.data) >= 1) then
if(new.tipus = "H") then
update saldo_diari set saldo = saldo + new.valor where data = new.data;
else 
update saldo_diari set saldo = saldo + new.valor where data = new.data;
end if;
else
	if(new.tipus = "H") then
    insert into saldo_diari values (date(new.data), saldo_Anterior+new.valor);
    else 
    insert into saldo_diari values (date(new.data), saldo_Anterior-new.valor);
    end if;
end if;
if(new.tipus = "H") then
set descr = concat (new.descripcio, '+', new.valor);
else
set descr = concat (new.descripcio, "-", new.valor);
end if;
insert into seguretat values(new.compte, new.data, user(), descr);
end
//
DELIMITER ;
INSERT INTO moviment VALUES ('aloha', now(), 'Ingrés ', 'H', 30);
INSERT INTO moviment VALUES ('aloha', now(), 'Ingrés ', 'H', 30);
INSERT INTO moviment VALUES ('aloha', now(), 'Reintegrament -', 'D', 30);
INSERT INTO moviment VALUES ('aloha', '2022-03-22', 'Reintegrament -', 'D', 30);
select * from seguretat;
select * from saldo;
select * from saldo_diari;