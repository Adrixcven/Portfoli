...
CREATE TABLE IF NOT EXISTS total
  (nom_proj CHAR(20) NOT NULL UNIQUE,
  acabado CHAR(100) NOT NULL
) ENGINE=InnoDB;

DELIMITER //
CREATE TRIGGER ej1 after insert ON dbuoc.projectes FOR EACH ROW
begin
if ((select count(*) from total where nom_proj = new.nom_proj) = 0) then
insert into total value (new.nom_proj, " ");
else
if ((select data_fi from dbuoc.projectes where nom_proj = new.nom_proj) is null) then
	update total set acabado = "No ha terminado" where nom_proj = new.nom_proj;
else
	update total set acabado = "Ha terminado" where nom_proj = new.nom_proj;
end if;
end if;
end
//
